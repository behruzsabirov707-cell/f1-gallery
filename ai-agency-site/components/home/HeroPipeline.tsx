...
export default function HeroPipeline({
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute left-[12.5%] right-[12.5%] top-6 hidden h-px bg-gradient-to-r from-(--color-brand-from)/60 to-(--color-brand-to)/60 sm:block">
        <span className="pipeline-packet absolute top-1/2 h-2 w-2 -translate-y-1/2 rounded-full bg-(--color-brand-to) shadow-[0_0_8px_2px_rgba(63,160,245,0.6)]" />
      </div>
      {labels.map((label, i) => (
        <div
          key={label}
