...
export default function ServiceCard({
  ...
  return (
    <Link
      href={href}
      className="group flex h-full flex-col gap-4 rounded-xl border border-(--color-border) bg-(--color-bg-elevated) p-6 transition-all duration-300 hover:-translate-y-1 hover:border-(--color-brand-to)/40 hover:shadow-[0_20px_50px_-24px_rgba(63,160,245,0.35)]"
    >
        ...
        {tools.map((tool) => {
          ...
          return (
            <ToolIcon
              key={tool.label}
              title={tool.label}
              className="h-4 w-4 text-(--color-ink-muted) transition-colors duration-300 group-hover:text-(--color-brand-to)"
            />
          );
        })}
      </div>
